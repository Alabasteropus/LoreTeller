...
do
    local font = CreateFont("LoreTellerNameFont")
...
do
    local font = CreateFont("LoreTellerButtonFont")
...
function SoundQueueUI:InitDisplay()
    self.frame = CreateFrame("Frame", "LoreTellerFrame", UIParent, "BackdropTemplate")
    ...
    self.frame.background:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\BackgroundGradient]])
    ...
    self.frame.resizer:SetNormalTexture([[Interface\AddOns\AI_LoreTeller\Textures\SizeGrabber-Up]])
    self.frame.resizer:SetPushedTexture([[Interface\AddOns\AI_LoreTeller\Textures\SizeGrabber-Down]])
    self.frame.resizer:SetHighlightTexture([[Interface\AddOns\AI_LoreTeller\Textures\SizeGrabber-Highlight]])
    ...
    self.frame.container.name = self.frame.container:CreateFontString(nil, "ARTWORK", "LoreTellerNameFont")
    ...
    function self.frame.container.stopGossip:SetGossipCount(gossipCount)
        local texture = gossipCount > 1 and [[Interface\AddOns\AI_LoreTeller\Textures\StopGossipMore]] or [[Interface\AddOns\AI_LoreTeller\Textures\StopGossip]]
...
function SoundQueueUI:InitPortraitLine()
    ...
    self.frame.portraitLine:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
    ...
    self.frame.miniPause:SetNormalTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
    ...
    self.frame.miniPause:SetPushedTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
    ...
    self.frame.miniPause.background:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SettingsButton]])
...
function SoundQueueUI:InitPortrait()
    ...
    self.frame.portrait.background:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameBackground]])
    ...
    self.frame.portrait.book:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\Book]])
    ...
    self.frame.portrait.pause.background:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameBackground]])
    self.frame.portrait.pause.background:SetAlpha(0.75)
    self.frame.portrait.pause:SetNormalTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
    ...
    self.frame.portrait.pause:SetPushedTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
    ...
    self.frame.portrait.border.texture:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
...
function SoundQueueUI:InitMover()
    ...
    self.frame.mover:SetNormalTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
    ...
    self.frame.mover:SetPushedTexture([[Interface\AddOns\AI_LoreTeller\Textures\PortraitFrameAtlas]])
    ...
    self.frame.mover.background:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SettingsButton]])
...
function SoundQueueUI:InitMinimapButton()
    ...
    local object = LibDataBroker:NewDataObject("LoreTeller", {
        type = "launcher",
        text = "LoreTeller",
        icon = [[Interface\AddOns\AI_LoreTeller\Textures\MinimapButton]],

        OnClick = function(self, button)
            -- Shift+Left Click toggles UI lock
            if button == "LeftButton" and IsShiftKeyDown() then
                Addon.db.profile.SoundQueueUI.LockFrame = not Addon.db.profile.SoundQueueUI.LockFrame
                SoundQueueUI:RefreshConfig()
                LoreDisplayUI:RefreshLockState()
                local state = Addon.db.profile.SoundQueueUI.LockFrame and "locked" or "unlocked"
                PlaySound(SOUNDKIT.U_CHAT_SCROLL_BUTTON)
                print("|cff00ff00LoreTeller:|r UI frames " .. state)
                return
            end
        ...
        OnTooltipShow = function(tooltip)
            ...
            local lockState = Addon.db.profile.SoundQueueUI.LockFrame and "Unlock" or "Lock"
            tooltip:AddLine(format("%sShift+Left Click:|r %s UI", GRAY_FONT_COLOR_CODE, lockState))
            tooltip:Show()
        end,
    })
    LibDBIcon:Register("LoreTeller", object, Addon.db.profile.MinimapButton.LibDBIcon)
end
...
function SoundQueueUI:RefreshConfig()
    ...
    LibDBIcon:Refresh("LoreTeller", Addon.db.profile.MinimapButton.LibDBIcon)
end
...
function SoundQueueUI:CreateButton(i)
    ...
    button.textWidget = button:CreateFontString(nil, "OVERLAY", "LoreTellerButtonFont")
    ...
    function button:Update(pushed, hovered)
        ...
        if hovered then
            ...
            self.textWidget:SetShadowColor(0, 0, 0, 1)
            self.iconWidget:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SoundQueueBulletDelete]])
            ...
            if isBeingPlayed then
                ...
                if Enums.SoundEvent:IsQuestEvent(event) then
                    ...
                    if event == Enums.SoundEvent.QuestAccept then
                        self.iconWidget:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SoundQueueBulletAccept]])
                    elseif event == Enums.SoundEvent.QuestProgress then
                        self.iconWidget:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SoundQueueBulletProgress]])
                    elseif event == Enums.SoundEvent.QuestComplete then
                        self.iconWidget:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SoundQueueBulletComplete]])
                    end
                elseif Enums.SoundEvent:IsGossipEvent(event) then
                    self.textWidget:SetTextColor(1, 1, 1)
                    self.iconWidget:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SoundQueueBulletGossip]])
                end
                self.iconWidget:SetSize(14, 14)
            else
                self.textWidget:SetTextColor(123 / 255, 147 / 255, 167 / 255)
                self.iconWidget:SetTexture([[Interface\AddOns\AI_LoreTeller\Textures\SoundQueueBulletQueue]])
...
function SoundQueueUI:UpdateSoundQueueDisplay()
    -- Hide the standard UI for lore events (they use LoreDisplayUI instead)
    local currentSound = SoundQueue:GetCurrentSound()
    local isLoreEvent = currentSound and Enums.SoundEvent:IsLoreEvent(currentSound.event)
    self.frame:SetShown(not Addon.db.profile.SoundQueueUI.HideFrame and not SoundQueue:IsEmpty() and not isLoreEvent)
