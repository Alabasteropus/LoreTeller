...
---@field ModuleVersion number Module's data format version (TOC ##X-LoreTeller-DataModule-Version, must be CURRENT_MODULE_VERSION to be loaded)
---@field ModulePriority number Module's priority (TOC ##X-LoreTeller-DataModule-Priority, larger number = higher priority)
...
---@field Maps number[] Map IDs in which the module should load (TOC ##X-LoreTeller-DataModule-Maps)

---@class DataModule
---@field METADATA DataModuleMetadata
---@field GetSoundPath fun(self: DataModule, fileName: string, event: SoundEvent): string Function implemented in the module that returns the sound path for the desired LoreTeller
...
DataModules =
{
    ...
    availableModules = {
        {
            AddonName = "LoreTellerData_Vanilla",
            Title = "LoreTeller Data - Vanilla",
            ContentVersion = "1.0.0",
            RelevantAboveVersion = 0,
            URL = "https://www.curseforge.com/wow/addons/loreteller",
        },
    },
}
...
function DataModules:Register(name, module)
    ...
    local moduleVersion = assert(tonumber(GetAddOnMetadata(name, "X-LoreTeller-DataModule-Version")),
    ...
    -- Validate zone lore data if present
    self:ValidateZoneLoreData(module)
...
---Validates zone lore data entries in a module
---@param module DataModule Module data table
function DataModules:ValidateZoneLoreData(module)
    -- Check if module has ZoneLoreData
    if not module.ZoneLoreData then
        return
    end

    local moduleName = module.METADATA and module.METADATA.AddonName or "Unknown"
    
    for zoneID, entries in pairs(module.ZoneLoreData) do
        if type(entries) ~= "table" then
            Addon:Print(format("Warning: Module '%s' has invalid ZoneLoreData for zone %s (not a table)", moduleName, tostring(zoneID)))
        else
            for i, entry in ipairs(entries) do
                ...
                if not entry.zoneID then
                    table.insert(missingFields, "zoneID")
                end
                if not entry.zoneName or entry.zoneName == "" then
                    table.insert(missingFields, "zoneName")
                end
                if not entry.loreText or entry.loreText == "" then
                    table.insert(missingFields, "loreText")
                end
                if not entry.audioFile or entry.audioFile == "" then
                    table.insert(missingFields, "audioFile")
                end
                if not entry.source or entry.source == "" then
                    table.insert(missingFields, "source")
                end
                
                if #missingFields > 0 then
                    local entryDesc = entry.subZone and format("POI '%s'", entry.subZone) or "zone entry"
                    Addon:Print(format("Warning: Module '%s' zone %d %s (index %d) is missing required fields: %s", 
                        moduleName, zoneID, entryDesc, i, table.concat(missingFields, ", ")))
                end
            end
        end
    end
...
function DataModules:EnumerateAddons()
    ...
    for i = 1, GetNumAddOns() do
        local moduleVersion = tonumber(GetAddOnMetadata(i, "X-LoreTeller-DataModule-Version"))
        if moduleVersion and (FORCE_ENABLE_DISABLED_MODULES or GetAddOnEnableState(playerName, i) ~= 0) then
            local name = GetAddOnInfo(i)
            local mapsString = GetAddOnMetadata(i, "X-LoreTeller-DataModule-Maps")
            local maps = {}
            if mapsString then
            ...
            local module =
            {
                ...
                ModulePriority = tonumber(GetAddOnMetadata(name, "X-LoreTeller-DataModule-Priority")) or 0,
...
local getFileNameForEvent =
{
    ...
    [Enums.SoundEvent.ZoneLore]      = function(soundData) return soundData.fileName end,
    [Enums.SoundEvent.POILore]       = function(soundData) return soundData.fileName end,
}
setmetatable(getFileNameForEvent,
    {
        __index = function(self, event)
            error(format([[Unhandled LoreTeller sound event %d "%s"]], event,
