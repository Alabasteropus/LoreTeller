...
---@field unitIsObjectOrItem? boolean Whether the NPC that the voiceover originates from is a GameObject or Item
...
---@field handle? number Sound handle assigned by `Utils:PlaySound(soundData)`
---@field nextSoundTimer? any Handle to the `AceTimer` timer that progresses the queue
---@field fileName? string Name of the sound file to be played
---@field filePath? string Path to the sound file to be played
---@field length? number Duration of sound file to be played in seconds
---@field module? DataModule Data module that provided the sound file
...
function SoundQueue:AddSoundToQueue(soundData)
    ...
    -- Don't play lore if there are quest or gossip sounds in the queue
    if Enums.SoundEvent:IsLoreEvent(soundData.event) then
        for _, queuedSound in ipairs(self.sounds) do
            if Enums.SoundEvent:IsQuestEvent(queuedSound.event) or Enums.SoundEvent:IsGossipEvent(queuedSound.event) then
                return
            end
        end
    end
